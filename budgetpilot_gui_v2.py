#!/usr/bin/env python3
import json
import subprocess
from pathlib import Path
from datetime import date
import tkinter as tk
from tkinter import ttk

BASE = Path.home() / "BudgetPilot"
DATA = BASE / "data"
SETTINGS = DATA / "settings.json"
INCOMES = DATA / "incomes.json"
PAYMENTS = DATA / "payments.json"
EXPENSES = DATA / "expenses.json"

DATA.mkdir(parents=True, exist_ok=True)

def load(p, d):
    if not p.exists():
        p.write_text(json.dumps(d, indent=2))
    return json.loads(p.read_text())

def save(p, d):
    p.write_text(json.dumps(d, indent=2))

def run_core(args=None):
    try:
        cmd = [str(BASE / "budgetpilot.py")]
        if args:
            cmd += args
        return subprocess.check_output(cmd, text=True)
    except Exception as e:
        return str(e)

# ---------- APP ----------
root = tk.Tk()
root.title("BudgetPilot v2")
root.geometry("1200x800")

main = ttk.Frame(root)
main.pack(fill="both", expand=True)

# ---------- LEFT (INPUT) ----------
left = ttk.Frame(main, width=400)
left.pack(side="left", fill="y", padx=10, pady=10)

# ---------- RIGHT (OUTPUT) ----------
right = ttk.Frame(main)
right.pack(side="right", fill="both", expand=True, padx=10, pady=10)

# ---------- DASHBOARD ----------
dash = ttk.Frame(right)
dash.pack(fill="x")

lbl_money = ttk.Label(dash, text="€ 0", font=("Arial", 26, "bold"))
lbl_money.pack(anchor="w")

lbl_day = ttk.Label(dash, text="€/deň: 0", font=("Arial", 14))
lbl_day.pack(anchor="w")

lbl_status = ttk.Label(dash, text="STAV", font=("Arial", 14))
lbl_status.pack(anchor="w")

# ---------- OUTPUT ----------
output = tk.Text(right, height=25, font=("Monospace", 10))
output.pack(fill="both", expand=True)

def refresh():
    out = run_core()
    output.delete("1.0", tk.END)
    output.insert(tk.END, out)

    # veľmi jednoduchý parse
    for line in out.splitlines():
        if "Použiteľné peniaze" in line:
            lbl_money.config(text=line.split(":")[1].strip())
        if "Na deň" in line:
            lbl_day.config(text=line.split(":")[1].strip())
        if "Stav" in line:
            lbl_status.config(text=line.split(":")[1].strip())

# ---------- ÚČET ----------
box = ttk.LabelFrame(left, text="Účet + rezerva")
box.pack(fill="x", pady=5)

balance = ttk.Entry(box)
balance.insert(0, "2000")
balance.pack(fill="x", pady=3)

reserve_enabled = tk.BooleanVar()
ttk.Checkbutton(box, text="Použiť rezervu", variable=reserve_enabled).pack(anchor="w")

reserve = ttk.Entry(box)
reserve.insert(0, "500")
reserve.pack(fill="x", pady=3)

def save_account():
    save(SETTINGS, {
        "account_balance": float(balance.get()),
        "use_reserve": reserve_enabled.get(),
        "safe_min": float(reserve.get() or 0)
    })
    refresh()

ttk.Button(box, text="Uložiť", command=save_account).pack(fill="x", pady=5)

# ---------- PRÍJEM ----------
box = ttk.LabelFrame(left, text="Príjem")
box.pack(fill="x", pady=5)

inc_name = ttk.Entry(box)
inc_name.insert(0, "Výplata")
inc_name.pack(fill="x")

inc_amount = ttk.Entry(box)
inc_amount.insert(0, "2000")
inc_amount.pack(fill="x")

inc_day = ttk.Spinbox(box, from_=1, to=31)
inc_day.set(15)
inc_day.pack(fill="x")

def add_income():
    data = load(INCOMES, [])
    data.append({
        "name": inc_name.get(),
        "amount": float(inc_amount.get()),
        "day": int(inc_day.get()),
        "frequency": "monthly",
        "start": "2026-01-01"
    })
    save(INCOMES, data)
    refresh()

ttk.Button(box, text="Pridať príjem", command=add_income).pack(fill="x", pady=5)

# ---------- PLATBA ----------
box = ttk.LabelFrame(left, text="Pravidelná platba")
box.pack(fill="x", pady=5)

pay_type = ttk.Combobox(box, values=[
    "Hypotéka","Elektrina","Voda","PZP","Internet","Iné"
])
pay_type.set("Hypotéka")
pay_type.pack(fill="x")

pay_name = ttk.Entry(box)
pay_name.pack(fill="x")

pay_amount = ttk.Entry(box)
pay_amount.insert(0, "100")
pay_amount.pack(fill="x")

pay_day = ttk.Spinbox(box, from_=1, to=31)
pay_day.set(1)
pay_day.pack(fill="x")

freq = ttk.Combobox(box, values=["monthly","quarterly","yearly"])
freq.set("monthly")
freq.pack(fill="x")

def add_payment():
    name = pay_name.get() if pay_type.get()=="Iné" else pay_type.get()
    data = load(PAYMENTS, [])
    data.append({
        "name": name,
        "amount": float(pay_amount.get()),
        "day": int(pay_day.get()),
        "frequency": freq.get(),
        "start": "2026-01-01"
    })
    save(PAYMENTS, data)
    refresh()

ttk.Button(box, text="Pridať platbu", command=add_payment).pack(fill="x", pady=5)

# ---------- RÝCHLY VÝDAVOK ----------
box = ttk.LabelFrame(left, text="Rýchly výdavok")
box.pack(fill="x", pady=5)

quick_amount = ttk.Entry(box)
quick_amount.insert(0, "50")
quick_amount.pack(fill="x")

def quick_expense():
    data = load(EXPENSES, [])
    data.append({
        "name": "rýchly výdavok",
        "amount": float(quick_amount.get()),
        "date": date.today().isoformat()
    })
    save(EXPENSES, data)
    refresh()

ttk.Button(box, text="Pridať rýchly výdavok", command=quick_expense).pack(fill="x", pady=5)

# ---------- DETAILNÝ VÝDAVOK ----------
box = ttk.LabelFrame(left, text="Detailný výdavok")
box.pack(fill="x", pady=5)

exp_name = ttk.Combobox(box, values=[
    "Potraviny","Nafta","Večera","Deti","Lekáreň","Iné"
])
exp_name.set("Potraviny")
exp_name.pack(fill="x")

exp_amount = ttk.Entry(box)
exp_amount.insert(0, "100")
exp_amount.pack(fill="x")

exp_date = ttk.Entry(box)
exp_date.insert(0, date.today().isoformat())
exp_date.pack(fill="x")

def add_expense():
    name = exp_name.get()
    data = load(EXPENSES, [])
    data.append({
        "name": name,
        "amount": float(exp_amount.get()),
        "date": exp_date.get()
    })
    save(EXPENSES, data)
    refresh()

ttk.Button(box, text="Pridať výdavok", command=add_expense).pack(fill="x", pady=5)

refresh()
root.mainloop()
